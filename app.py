...
def get_dynamic_descriptions(tickers):
    """
    Queries Yahoo Finance metadata dynamically to generate clean 1-word labels.
    Handles extraction differences between general individual stocks and sector ETFs.
    """
    descriptions = {}
    # Suffixes to drop from names to ensure we capture the actual descriptive industry word
    noise_words = {"SELECT", "SECTOR", "SPDR", "FUND", "INDEX", "ETF", "INC", "CORP", "SHARED"}
    ...
    for t in tickers:
        try:
            ...
            # Use 'shortName' for individual stocks, fallback to 'longName' for sector ETFs
            raw_name = info.get("shortName") or info.get("longName") or ""
            
            if raw_name:
                # Clean and split string into alpha-only uppercase blocks
                words = [w.strip("(),.").upper() for w in raw_name.split() if w.strip("(),.")]
                # Filter out boring structural financial nomenclature
                meaningful_words = [w for w in words if w not in noise_words]
                
                if meaningful_words:
                    # Capture the primary sector identifier word (Title-cased)
                    core_desc = meaningful_words[0].lower().capitalize()
                    descriptions[t] = f"{t} ({core_desc})"
                    continue
            
            descriptions[t] = t
        except Exception:
            descriptions[t] = t  # Safe fallback if API limits or offline errors occur
...
if trigger_go:
 ...
 if not parsed_tickers or not bench_ticker:
     ...
     with st.spinner("Analyzing market momentum fields and generating clean vectors..."):
         ...
         # Dynamically pull ticker metadata directly from Yahoo Finance API
         label_mapping = get_dynamic_descriptions(parsed_tickers)
 ...
 if not raw_rrg_data:
     ...
     for idx, (ticker, df) in enumerate(raw_rrg_data.items()):
         ...
         # Match coordinates with our dynamically fetched description label
         display_name = label_mapping.get(ticker, ticker)
     ...
     if not all_x or not all_y:
        ...
        fig.add_annotation(x=100 + (max_dev/2), y=100 + (max_dev/2), text="LEADING",
        font=dict(color="green", size=16), showarrow=False)fig.add_annotation(x=100 + (max_dev/2), y=100 - (max_dev/2), text="WEAKENING",
        font=dict(color="gold", size=16), showarrow=False)fig.add_annotation(x=100 - (max_dev/2), y=100 - (max_dev/2), text="LAGGING", 
        font=dict(color="red", size=16), showarrow=False)fig.add_annotation(x=100 - (max_dev/2), y=100 + (max_dev/2), text="IMPROVING", 
        font=dict(color="blue", size=16), showarrow=False)
        ...
        st.plotly_chart(fig, use_container_width=True)else:st.info("Configure variables inside left side panel and click 'Render RRG Chart' to track structural transformations.")
